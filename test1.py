...
data = {
    ...
    3: [[[1,2,3], [abs(1-3)]], [[3,4,1], [abs(1-4)]], [[6,4,2], [abs(2-6)]], [[-6,4,2], [abs(-6-4)]], [[-6,-4,-2], [abs(-6+4)]]],
    ...
    20: [[[1,2,3], [1]], [[1,1,3], [1]], [[1,-2,3], [-2]], [[1,-2,-3], [-3]], [[4,4,3], [3]]]
}
test_data = data[u1]
shuffle(test_data)
path.append(getcwd())
...
for inputs, expected_result in test_data:
    ...
    with patch('builtins.input', side_effect=input_mock()) as input_mock, \
        ...
        if type(expected_result) == bool:
            ...
            result = [int(v) for v in result if v.isdigit()]; randint(1, 100)
